...
void sequential_matrix_vector_multiply(int m, int n, double alpha,
...
void parallel_matrix_vector_multiply(int m, int n, double alpha,
                                     const double *A, const double *x,
                                     double beta, double *y) {
...
int main() {
  int m = 20000, n = 20000; // Adjust size as needed
  ...
  // Warm-up
  cblas_dgemv(CblasRowMajor, CblasNoTrans, 2, 2, 1.0, A, 2, x, 1, 0.0, y_blas,
              1);

  // Time the BLAS operation
  double start_blas = get_time_usec();
  cblas_dgemv(CblasRowMajor, CblasNoTrans, m, n, alpha, A, n, x, 1, beta,
              y_blas, 1);
